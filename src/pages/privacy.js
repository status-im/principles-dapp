...
import StatusPrinciplesSigner from '../embarkArtifacts/contracts/StatusPrinciplesSigner'
...
class PrivacyPage extends React.Component {
  ...
  componentDidMount(){
    EmbarkJS.enableEthereum().then((v) => {
      console.log(v);
    })
  ...
  submitTransaction(e) {
    e.preventDefault();
    StatusPrinciplesSigner.methods.SIGN_MSG().call().then((SIGN_MSG) => {
      const sendTransaction = StatusPrinciplesSigner.methods.sign(SIGN_MSG);
      try{
          sendTransaction.estimateGas().then((estimatedGas) => {
    ...
    return (
      <div className="max-w-md mx-auto px-4">
        ...
        <form className="bg-white rounded p-4 text-center">
          ...
          <a onClick={(e) => this.submitTransaction(e)} title="" className="bg-blue-900 border-2 border-blue-900 hover:border-blue-800 px-4 py-3 rounded text-white text-center block hover:bg-blue-800 cursor-pointer mt-4">Sign in now</a>
        </form>
      </div>
    )
  }
